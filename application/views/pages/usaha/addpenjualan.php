				...
				<div class="card">
					<div class="card-body p-5">
						<div class="row border-bottom pb-4">
							<div class="col-6">
								<div class="text-muted mb-2">Tanggal Penjualan</div>
						...
						<div class="row mt-3">
							...
							<div class="col-lg-4 col-md-12 my-2">
								<div class="text-muted mb-2">Pembelian</div>
								<select class="form-control" data-toggle="select" name="pembelian" title="Simple select" data-live-search="true" data-live-search-placeholder="Search...">
							...
							<div class="col-lg-4 col-md-12 my-2">
								...
								<select class="form-control" data-toggle="select" name="status_pembayaran" title="Simple select" data-live-search="true" data-live-search-placeholder="Search...">
									<option selected disabled>LUNAS / BELUM LUNAS</option>
						...
						<table class="table border-bottom table-flush mt-3" id="dynamicAddRemove">
							...
							<tbody>
								<tr>
									...
									<td class="align-middle mb-0 text-sm font-weight-600">
										<select class="form-control" data-toggle="select" name="produk[0]" title="Simple select" data-live-search="true" data-live-search-placeholder="Search ...">
											...
											<?php foreach ($data as $key => $row) : ?>
												<option value="<?= $row['id']; ?>"><?= $row['nama']; ?> | Stok : <?= $row['jumlah']; ?></option>
											<?php endforeach ?>
										</select>
									</td>
									<td class="align-middle mb-0 text-sm font-weight-600">
									...
									<td class="align-middle text-right mb-0 text-sm font-weight-600">
										<input class="form-control rupiahInput" style="text-align: right;" type="text" name="harga[0]" placeholder="Harga Jual">
									</td>
								</tr>
							</tbody>
						</table>
...
<script>
	$('.postProduk').submit(function(e) {
		...
		$.ajax({
			type: 'POST',
			url: '<?= base_url('usaha/addProdukPenjualan'); ?>',
			data: formData,
			success: function(response) {
				...
				$('input[name="datas"]').val(JSON.stringify(res.data));
			},
	...
	$('.save-db').submit(function(e) {
		...
		$.ajax({
			type: 'POST',
			url: '<?= base_url('usaha/addPenjualanDB'); ?>',
			data: formData,
			success: function(response) {
				let res = JSON.parse(response);
				...
				if (res.status === 200) {
					Swal.fire({
						icon: 'success',
						title: 'Success!',
						text: `Berhasil tambah penjualan`,
					}).then((result) => {
						if (result.isConfirmed || result.isDismissed) {
							location.replace('<?= base_url('usaha/penjualan') ?>');
						}
					});
				} else {
					Swal.fire({
...
<script type="text/javascript">
	...
	$("#dynamic-ar").click(function() {
		...
		var newRow = `<tr>
						...
						<td class="align-middle text-right mb-0 text-sm font-weight-600">
							<input class="form-control rupiahInput" style="text-align: right;" type="text" name="harga[${i}]" placeholder="Harga Jual">
						</td>
					</tr>`;
